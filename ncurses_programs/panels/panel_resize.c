...
static void
set_user_ptrs(PANEL **panels, PANEL_DATA * ptrs, int n)
{
    WINDOW *win;
    int x, y, w, h, i;
    char temp[80];
...
main(void)
{
    ...
    if ((ptrs = (PANEL_DATA *) calloc((size_t) nptrs, sizeof(PANEL_DATA)))
        == NULL) {
        perror("panel data");
        exit(EXIT_FAILURE);
    }
    ...
    set_user_ptrs(my_panels, ptrs, nptrs);
    ...
    while ((ch = getch()) != KEY_F(1)) {
        ...
        if (size && resized) {
            resized = FALSE;
            if ((old_win = panel_window(stack_top)) != NULL
                && (temp_win = newwin(newh, neww, newy, newx)) != NULL) {
