...
static void
display(WINDOW *win, state ** area, int startx, int starty, int endx, int endy)
{
    int i, j;
    wclear(win);
...
static void
update_state(state ** area, int startx, int starty, int endx, int endy)
{
    int i, j;
...
main(void)
{
    ...
    if ((workarea = calloc((size_t) COLS, sizeof(state *))) == NULL) {
        endwin();
        return EXIT_FAILURE;
    }
    for (i = 0; i < COLS; ++i) {
        if ((workarea[i] = calloc((size_t) LINES, sizeof(state))) == NULL) {
            endwin();
            return EXIT_FAILURE;
        }
    }
